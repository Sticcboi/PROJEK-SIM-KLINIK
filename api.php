...
namespace Tqdev\PhpCrudApi\Middleware\Router {
    ...
    class SimpleRouter implements Router
    {
        ...
        private function getRouteNumbers(ServerRequestInterface $request): array
        {
            ...
            $path = array();
            // If a `table` query parameter is provided, rewrite path to /records/{table}
            $params = RequestUtils::getParams($request);
            if (isset($params['table']) && $params['table'] !== '') {
                $table = is_array($params['table']) ? $params['table'][0] : $params['table'];
                $currentFirst = RequestUtils::getPathSegment($request, 1);
                if ($currentFirst !== 'records') {
                    $newPath = '/records/' . $table;
...
namespace Tqdev\PhpCrudApi {
    ...
    class RequestUtils
    {
        ...
        {
            // Allow overriding the table name with the query parameter `table`
            if ($part === 2) {
                $params = self::getParams($request);
                if (isset($params['table']) && $params['table'] !== '') {
                    return is_array($params['table']) ? $params['table'][0] : $params['table'];
                }
            }
            $path = $request->getUri()->getPath();
...
namespace Tqdev\PhpCrudApi {
    ...
    'username' => 'root',
    'password' => '',
    'database' => 'db_simklinik', // Database 13 tabel kita kemarin
    'debug'    => true 
    ]);
